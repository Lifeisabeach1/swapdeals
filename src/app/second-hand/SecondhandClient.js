...
const ADVANTAGES = [
  'Spara 50-90% jämfört med nypris på märkesvaror och designerprodukter',
  'Minska ditt klimatavtryck dramatiskt - ett begagnat plagg sparar 25 kg CO2',
  'Hitta unika vintage-fynd och saker som ingen annan har',
  'Ofta bättre kvalitet - äldre saker tillverkades för att hålla',
  'Skapa en personlig stil som verkligen är din egen'
];

const DESIGNER_BRANDS = [
  { category: 'Klassiska lyx', brands: 'Burberry, Hermès, Chanel, Louis Vuitton - håller värdet och kvaliteten' },
  { category: 'Skandinavisk design', brands: 'Acne Studios, Filippa K, Tiger of Sweden, Ganni' },
  { category: 'Vintage-guld', brands: 'Marimekko, Levi\'s 501 från 80-90-talet, Ralph Lauren' },
  { category: 'Skor', brands: 'Dr Martens, Birkenstock, kvalitetsskor i läder håller åratal' }
];

const ELECTRONICS_AVOID = [
  'Batteridrivna enheter med dålig batterihälsa - byte är ofta dyrt',
  'Vitvaror utan garanti - reparationer kan kosta mer än nypris',
  'Mycket gammal elektronik (10+ år) - ofta inte kompatibel med modern teknik',
  'Saknar laddare eller kablar som är svåra att hitta'
];

const SUMMARY_TIPS = [
  { title: '1. Lär dig kvalitet', desc: 'Känn på tyger, kolla sömmar, testa konstruktion' },
  { title: '2. Timing är nyckeln', desc: 'Kom tidigt, besök ofta, tänk säsong' },
  { title: '3. Ha tålamod', desc: 'Bästa fynden kräver tid att hitta' },
  { title: '4. Testa alltid', desc: 'Prova kläder, testa elektronik, kontrollera möbler' },
  { title: '5. Tänk långsiktigt', desc: 'Välj kvalitet som håller över snabba trender' },
  { title: '6. Ha roligt!', desc: 'Second hand-jakt är en upplevelse - njut av resan' }
];
  ...
  <ul className="space-y-3 text-gray-700">
    {items.map((item, index) => (
      <li key={index} className="flex items-start">
        <span className="mr-3 mt-1.5 flex-shrink-0 w-1.5 h-1.5 bg-green-600 rounded-full" aria-hidden="true"></span>
        <span>{item}</span>
      </li>
    ))}
  </ul>
...
const DesignerBrandsList = ({ brands }) => (
  <ul className="space-y-3 text-gray-700">
    {brands.map((item, index) => (
      <li key={index} className="flex items-start">
        <span className="mr-3 mt-1.5 flex-shrink-0 w-1.5 h-1.5 bg-green-600 rounded-full" aria-hidden="true"></span>
        <span>
          <span className="font-semibold">{item.category}:</span> {item.brands}
        </span>
      </li>
    ))}
  </ul>
...
const InfoBox = ({ title, children, highlight }) => (
  <div className={`my-10 p-8 bg-gray-50 rounded-lg ${highlight ? 'border-l-4 border-green-600' : ''}`}>
...
export default function BlogPost() {
  const advantages = useMemo(() => ADVANTAGES, []);
  const designerBrands = useMemo(() => DESIGNER_BRANDS, []);
  const electronicsAvoid = useMemo(() => ELECTRONICS_AVOID, []);
  const summaryTips = useMemo(() => SUMMARY_TIPS, []);
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <article>
          ...
          <div className="prose prose-lg max-w-none">
            ...
            <figure className="float-right ml-8 mb-6 w-80 rounded-lg overflow-hidden shadow-lg">
              <Image 
            ...
            <section>
              ...
              <InfoBox title="De största fördelarna med second hand:" highlight>
            ...
            <section>
              ...
              <InfoBox title="Designermärken att leta efter:" highlight>
                <DesignerBrandsList brands={designerBrands} />
              </InfoBox>
            </section>
            ...
            <section>
              ...
              <InfoBox title="Elektronik att undvika second hand:" highlight>
                <BulletList items={electronicsAvoid} />
              </InfoBox>
            </section>
              ...
              <InfoBox>
                ...
                <div className="grid md:grid-cols-2 gap-6 text-gray-700">
                  {summaryTips.map((tip, index) => (
                    <div key={index}>
                      <p className="font-semibold text-gray-900 mb-2">{tip.title}</p>
